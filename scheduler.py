# scheduler.py (Versi diubah untuk menggunakan EDA)
...
# --- PERUBAHAN 1: Ganti impor dari SHC ke EDA ---
from eda_algorithm import eda_optimize_schedule
...
RESULTS_FILE = 'eda_results.csv' # Nama file hasil diubah
EDA_ITERATIONS = 500 # Jumlah iterasi disesuaikan untuk EDA
...
def get_task_load(index: int):
    # Menggunakan rumus yang sama: endpoint^2 * 10000
...
# --- Eksekutor Tugas Asinkron (Tidak Berubah) ---
...
# --- Fungsi Paska-Proses & Metrik (Tidak Berubah Signifikan) ---

def write_results_to_csv(results_list: list):
    """Menyimpan hasil eksekusi ke file CSV."""
    # Menggunakan nama file yang diubah
    global RESULTS_FILE 
    if 'shc' in RESULTS_FILE:
        RESULTS_FILE = 'eda_results.csv'
...
def calculate_and_print_metrics(results_list: list, vms: list[VM], total_schedule_time: float):
    # Fungsi metrik tetap sama, bergantung pada pandas
    try:
...
async def main():
    ...
    # 2. Jalankan Algoritma Penjadwalan (EDA)
    # --- PERUBAHAN 2: Ganti panggilan fungsi ke EDA ---
    best_assignment = eda_optimize_schedule(tasks, vms, EDA_ITERATIONS)
    
    print("\nPenugasan Tugas Terbaik Ditemukan:")
    # EDA mengembalikan Dict[task_id: vm_name], seperti SHC
    for i in range(min(10, len(best_assignment))): 
        print(f"  - Tugas {i} (Endpoint {tasks_dict[i].index}) -> {best_assignment.get(i)}")
    if len(best_assignment) > 10:
        print("  - ... etc.")

    # 3. Siapkan Eksekusi (Tidak Berubah)
    results_list = []
